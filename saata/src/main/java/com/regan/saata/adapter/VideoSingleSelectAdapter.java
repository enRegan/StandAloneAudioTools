...
public class VideoSingleSelectAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {

    //上下文环境
    private Context mContext;
    //数据集合
    private ArrayList<VideoInfo> mData;
    private ArrayList<VideoInfo> selectedList;
    ...
    public VideoSingleSelectAdapter(Context context, final ArrayList<VideoInfo> data, TextView tvConfirm, final CheckDone checkDone) {
...
//                ((AppCompatActivity)mContext).finish();
    ...
    public interface CheckDone {
        void done(ArrayList<VideoInfo> data);
    }
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof AudioHolder) {
            ...
            ((AudioHolder) holder).bind(VideoInfo, position);
    ...
    class AudioHolder extends RecyclerView.ViewHolder {
        ...
        private void bind(final VideoInfo info, final int position) {
            this.info = info;
            this.position = position;
            if (ivIcon.getTag() != null && (int) ivIcon.getTag(R.id.tag_postiton) == position) {
                ivIcon.setImageBitmap((Bitmap) ivIcon.getTag(R.id.tag_bitmap));
            } else {
                new Thread(new Runnable() {
                    @Override
                    public void run() {
                        final Bitmap videoFrame = MediaTool.getVideoFrame(info.getPath(), 1000000);
                        new Handler(Looper.getMainLooper()).post(new Runnable() {
                            @Override
                            public void run() {
                                ivIcon.setImageBitmap(videoFrame);
                                ivIcon.setTag(R.id.tag_postiton, position);
                                ivIcon.setTag(R.id.tag_bitmap, videoFrame);
                            }
                        });
                    }
                }).start();
            }
//            Bitmap videoFrame = MediaTool.getVideoFrame(info.getPath(), 2000000);
//            ivIcon.setImageBitmap(videoFrame);
...
//            });
            cbChoice.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
//                    LogUtils.d(Constant.TAG, " cbChoice onClick position : " + position);
...
//                    LogUtils.d(Constant.TAG, " cbChoice  pos: " + pos + "  onCheckedChanged: " + isChecked);
                    if (isChecked) {
                        mCheckStates.clear();
