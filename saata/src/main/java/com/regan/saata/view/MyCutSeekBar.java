 ...
 *
 */
public class MyCutSeekBar extends View {
    ...
    private Drawable mLeftEmpty;        //左空白
    private Drawable mRightEmpty;        //后空白
    ...
    private int mMarginTop = 0;   //上方空位
    private int mMarginBottom = 0;   //下方空位
    private int mHeight = 0;   //整体高度
    ...
    private double defaultScreenHigh = 1000;  //默认后滑块位置百分比

    private boolean isEdit = false;     //输入框是否正在输入

    public MyCutSeekBar(Context context) {
        this(context, null);
    }

    public MyCutSeekBar(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public MyCutSeekBar(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
//        this.setBackgroundColor(Color.BLACK);

        Resources resources = getResources();
        notScrollBarBg = resources.getDrawable(R.drawable.cut_track_blue);
        hasScrollBarBg = resources.getDrawable(R.drawable.cut_track_top);
        mThumbLow = resources.getDrawable(R.drawable.cut_line);
        mThumbHigh = resources.getDrawable(R.drawable.cut_line);
        mLeftEmpty = resources.getDrawable(R.color.black30);
        mRightEmpty = resources.getDrawable(R.color.black30);

        mThumbLow.setState(STATE_NORMAL);
        mThumbHigh.setState(STATE_NORMAL);
        mLeftEmpty.setState(STATE_NORMAL);
        mRightEmpty.setState(STATE_NORMAL);

//        mScollBarWidth = PxUtils.dp2px(context, notScrollBarBg.getIntrinsicWidth());
//        mScollBarHeight = PxUtils.dp2px(context, notScrollBarBg.getIntrinsicHeight());
//
//        mThumbWidth = PxUtils.dp2px(context, mThumbLow.getIntrinsicWidth());
//        mThumbHeight = PxUtils.dp2px(context, mThumbLow.getIntrinsicHeight());

        mScollBarWidth = PxUtils.dp2px(context, notScrollBarBg.getIntrinsicWidth());
        mScollBarHeight = PxUtils.dp2px(context, notScrollBarBg.getIntrinsicHeight());

        mThumbWidth = PxUtils.dp2px(context, 5);
        mThumbHeight = PxUtils.dp2px(context, 200);

        mMarginTop = PxUtils.dp2px(context, 30);
        mMarginBottom = PxUtils.dp2px(context, 30);
        mHeight = PxUtils.dp2px(context, 260);
    }
...
//        int height = measureHeight(heightMeasureSpec);
        ...
        mOffsetLow = formatDouble(defaultScreenLow / 1000 * (mDistance)) + mThumbWidth / 2;
        mOffsetHigh = formatDouble(defaultScreenHigh / 1000 * (mDistance)) + mThumbWidth / 2;
...
//        text_Paint.setTextSize(20);
        ...
        int aaa = mThumbMarginTop + mThumbHeight / 2 - mScollBarHeight / 2 + mMarginTop;
...
//        notScrollBarBg.draw(canvas);
        ...
        //前滑块
        mThumbLow.setBounds((int) (mOffsetLow - mThumbWidth / 2), mThumbMarginTop + mMarginTop, (int) (mOffsetLow + mThumbWidth / 2), mThumbHeight + mThumbMarginTop + mMarginBottom);
        mThumbLow.draw(canvas);

        //后滑块
        mThumbHigh.setBounds((int) (mOffsetHigh - mThumbWidth / 2), mThumbMarginTop + mMarginTop, (int) (mOffsetHigh + mThumbWidth / 2), mThumbHeight + mThumbMarginTop + mMarginBottom);
        mThumbHigh.draw(canvas);

        //左边未选择空白
        mLeftEmpty.setBounds(0, mThumbMarginTop, (int) (mOffsetLow - mThumbWidth / 2), mHeight);
        mLeftEmpty.draw(canvas);

        //右边未选择空白
        mRightEmpty.setBounds((int) (mOffsetHigh + mThumbWidth / 2), mThumbMarginTop, mScollBarWidth, mHeight);
        mRightEmpty.draw(canvas);

        double progressLow = formatDouble((mOffsetLow - mThumbWidth / 2) * 1000 / mDistance);
        double progressHigh = formatDouble((mOffsetHigh - mThumbWidth / 2) * 1000 / mDistance);
...
//            }
    ...
    public void setProgressLow(double progressLow) {
        ...
        mOffsetLow = formatDouble(progressLow / 1000 * (mDistance)) + mThumbWidth / 2;
        isEdit = true;
        refresh();
    }
    ...
    public void setProgressHigh(double progressHigh) {
        ...
        mOffsetHigh = formatDouble(progressHigh / 1000 * (mDistance)) + mThumbWidth / 2;
        isEdit = true;
        refresh();
    }
    ...
    public interface OnSeekBarChangeListener {
        ...
        //滑动时
        public void onProgressChanged(MyCutSeekBar seekBar, double progressLow,
                                      double progressHigh);
